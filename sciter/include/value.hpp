  ...
  namespace sciter
  {
    ...
    public:
     ...
     ~value()                 { ValueClear(this); }
      ...
      value( const astring& s ) { aux::a2w as(s.c_str()); ValueInit(this); ValueStringDataSet(this, LPCWSTR(as.c_str()), UINT(as.length()), UT_SYMBOL); }
      //value( const std::ustring& s ) { ValueInit(this); ValueStringDataSet(this, LPCWSTR(s.c_str()), UINT(s.length()), 0); }
      //value( const std::string& s ) { aux::a2w as(s.c_str()); ValueInit(this); ValueStringDataSet(this, LPCWSTR(as.c_str()), UINT(as.length()), UT_SYMBOL); }
      ...
      { 
        aux::a2w as(s);
        ValueInit(this); ValueStringDataSet(this, LPCWSTR(as.c_str()), UINT(as.length()), UT_SYMBOL);
      }
      ...
      {
        value v;
        ValueStringDataSet(&v, LPCWSTR(s), UINT(slen), 2);
        return v;
      }
...
//#if defined(HAS_TISCRIPT)
      ...
      value call( int argc, const value* argv, value self = value(), const WCHAR* url_or_script_name = 0)
      {
        value rv;
        ValueInvoke(this,&self,argc,argv,&rv,LPCWSTR(url_or_script_name));
        return rv;
      }

      value call() {  return call(0,nullptr);  }
      value call( const value& p1 ) {  return call(1,&p1); }
      value call( const value& p1, const value& p2 ) { value args[2] = { p1,p2 };  return call(2,args); }
      value call( const value& p1, const value& p2, const value& p3 ) { value args[3] = { p1,p2,p3 };  return call(3,args); }
      value call( const value& p1, const value& p2, const value& p3, const value& p4 ) { value args[4] = { p1,p2,p3,p4 };  return call(4,args); }
...
//#endif //defined(HAS_TISCRIPT)
  ...
  {
    NATIVE_FUNCTOR_VALUE() {}
    virtual ~NATIVE_FUNCTOR_VALUE() {}
    virtual bool  invoke( unsigned int argc, const VALUE* argv, VALUE* retval) = 0;
  ...
  namespace sciter {
    ...
    inline value::value( const native_function_t& nfr ) {
      ValueInit(this); 
      ValueNativeFunctorSet(this, new native_function(nfr) );
    }
