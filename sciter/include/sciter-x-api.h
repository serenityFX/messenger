...
typedef struct _ISciterAPI {
  ...
  BOOL    SCFN( SciterEnumUrlData )(HWINDOW hWndSciter, URL_DATA_RECEIVER* receiver, LPVOID param, LPCSTR url);
#ifdef WINDOWS
  ...
  BOOL    SCFN( SciterDebugSetupClient )(
  ...
  BOOL   SCFN( SciterDebugAddBreakpoint )(
                HWINDOW     hwnd,      // HWINDOW of the sciter
                LPCWSTR  fileUrl,
                UINT     lineNo
              );
  BOOL   SCFN( SciterDebugRemoveBreakpoint )(
                HWINDOW     hwnd,      // HWINDOW of the sciter
                LPCWSTR  fileUrl,
                UINT     lineNo
              );
  BOOL   SCFN( SciterDebugEnumBreakpoints )(
                HWINDOW                     hwnd,      // HWINDOW of the sciter
                LPVOID                      param,     // param to be passed "as is" to the pfOutput
...
//|
  ...
  UINT SCFN( ValueNativeFunctorSet )( VALUE* pval, struct NATIVE_FUNCTOR_VALUE* pnfv);
...
// getting ISciterAPI reference:
  ...
  inline   VOID    SCAPI SciterSetCallback (HWINDOW hWndSciter, LPSciterHostCallback cb, LPVOID cbParam) { return SAPI()->SciterSetCallback (hWndSciter,cb,cbParam); }
  ...
  inline   VOID    SCAPI SciterUpdateWindow(HWINDOW hwnd) { return SAPI()->SciterUpdateWindow(hwnd); }
#ifdef WINDOWS
  ...
  inline  VOID    SCAPI SciterGetPPI (HWINDOW hWndSciter, UINT* px, UINT* py) { return SAPI()->SciterGetPPI (hWndSciter,px,py); }
  inline  BOOL    SCAPI SciterGetViewExpando ( HWINDOW hwnd, VALUE* pval ) { return SAPI()->SciterGetViewExpando ( hwnd, pval ); }
  inline  BOOL    SCAPI SciterEnumUrlData (HWINDOW hWndSciter, URL_DATA_RECEIVER* receiver, LPVOID param, LPCSTR url) { return SAPI()->SciterEnumUrlData (hWndSciter,receiver,param,url); }
#ifdef WINDOWS
  ...
  inline UINT SCAPI ValueNativeFunctorSet ( VALUE* pval, struct NATIVE_FUNCTOR_VALUE* pnfv) { return SAPI()->ValueNativeFunctorSet ( pval, pnfv); }
