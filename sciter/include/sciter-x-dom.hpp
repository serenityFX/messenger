...
/**DOM element.*/
  ...
  class editbox: public element
  {
    ...
    public: 
      ...
      bool replace(const WCHAR* text, size_t text_length)
      {
      ...
      sciter::string text_value() const
      {
        TEXT_VALUE_PARAMS sp(false);
        if( const_cast<editbox*>(this)->call_behavior_method(&sp) && sp.text && sp.length)
        {
            return sciter::string(sp.text, sp.length);
        }
        return sciter::string();
      }

      void text_value(const WCHAR* text, size_t length)
      {
      ...
      void text_value(const WCHAR* text)
      {
        TEXT_VALUE_PARAMS sp(true);
        sp.text = text;
        sp.length = text? UINT(str_length(text)):0;
        call_behavior_method(&sp);
        update();
      }
      ...
      int int_value( ) const
      {
          sciter::string s = text_value();
          aux::wchars text = aux::chars_of(s);
          return aux::to_int(text);
      }
